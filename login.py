...
def profile_screen():
    global info, transaction_button, wrongLabel, labell
    if labell:
    ...
    conn.commit()
    for row in c.fetchall():
        info = Label(text="Username: " + row[0] + " || Surname: " + row[1] + " || Mail: " + row[2] + " || Age: " + str(row[3]) + " || Password: " + row[4] + " || Balance: " + str(row[5]))
        info.pack()
    transaction_button = Button(text="Transaction", command=transaction)
    transaction_button.pack()


def transaction():
    global transaction_screen, withdraw, deposit, transfer
    transaction_screen = tkinter.Tk()
    transaction_screen.geometry("700x600")
    transaction_screen.title("Transaction")
    Label(transaction_screen, text="Welcome to the transaction page", fg='#000000').pack()
    # withdraw = Button(transaction_screen, text="Withdraw", command=withdraw_screen)
    # withdraw.pack()
    deposit = Button(transaction_screen, text="Deposit", command=deposit_screen)
    deposit.pack()
    # transfer = Button(transaction_screen, text="Transfer", command=transfer_screen)
    # transfer.pack()

def deposit_screen():
    global deposit_screenn, amount_entry, amount_label, deposit_button
    deposit_screenn = tkinter.Tk()
    deposit_screenn.geometry("700x600")
    deposit_screenn.title("Deposit")
    amount_entry = StringVar()
    Label(deposit_screenn, text="Welcome to the deposit page", fg='#000000').pack()
    Label(deposit_screenn, text="Amount: ", fg='#000000').pack()
    Entry(deposit_screenn, textvariable=amount_entry).pack()
    Button(deposit_screenn, text="Deposit", command=deposited).pack()
...
def deposited():
    ...
    if amount_entry.get().isdigit():
        for row in c.fetchall():
            balance = row[5]
        balance = balance + int(amount_entry.get())
        c.execute('''UPDATE user SET BALANCE = ? where GMAIL = ? and password = ?''',(balance, username1, password1))
        conn.commit()
        Label(deposit_screenn, text="Deposit Successful", fg='#000000').pack()
        c.execute('''SELECT * FROM user where GMAIL = ? and password = ?''',(username1, password1))
        conn.commit()
        for row in c.fetchall():
            Label(deposit_screenn, text="Username: " + row[0] + " || Surname: " + row[1] + " || Mail: " + row[2] + " || Age: " + str(row[3]) + " || Password: " + row[4] + " || Balance: " + str(row[5])).pack()
    else:
        print(type(amount_entry.get()),amount_entry.get(), type(amount_entry),)
        Label(deposit_screenn, text="Please enter an amount", fg='#000000').pack()
